...
#define led_pin_dry 11
#define all_ok_state 14
...
void task_adc(void *pvParameters)
{
 // printf("ADC task started\r\n");
  ...
  while (1) {
    bl_gpio_enable_output(led_pin_dry,0,0);
    bl_gpio_enable_output(all_ok_state,0,0);
    int16_t soil_moisture = read_adc();
    //printf("Soil humid readings: %"PRIu32"\r\n",soil_moisture);
   // vTaskDelay(5000 / portTICK_PERIOD_MS);
    ...
    if (soil_moisture>14000){
      bl_gpio_output_set(led_pin_dry,1);
      bl_gpio_output_set(all_ok_state,0);
    //  printf("Turn on water supply/ water supply is on\r\n");
    }
    ...
    // }
      ...
      bl_gpio_output_set(led_pin_dry,0);
      bl_gpio_output_set(all_ok_state,1);
   //   printf("All vitals found to be normal\r\n");
    }
    
//  if (ldr_readings>130){
//       printf("move plant to a better lighting conditions\r\n");
//     }
    vTaskDelay(5000 / portTICK_PERIOD_MS);
